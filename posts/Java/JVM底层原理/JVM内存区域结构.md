...
当我们新建一个String a="ab"时，JVM会先在字符串常量池中找是否存在key为"ab"的键值对
- 若不存在键值对，则字符串常量池会创建出一个**字面量->Byte数组**的键值对对象
- 在找到键值对后，String在**堆**中正常初始化，但是内部的value字段引用会通过键值对导向**字符串常量池**中1Byte数组，也就是说，String的**value字段指向的Byte[]并非存储在堆中，而是方法区中**
