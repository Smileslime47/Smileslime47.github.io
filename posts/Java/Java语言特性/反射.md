...
10
```
实际上，invoke非常接近**JVM**实际调用非静态方法的情形，当调用一个**非静态方法**时，实际上隐藏了一个参数，即当前对象的引用，当我们执行`s.charAt(0)`时，JVM看到的实际上是`String.charAt(s,0)`；而当我们直接调用静态方法；但是在class文件中，static方法并非传入了一个**null**，而是压根就**没有**这个参数
```java
public class test {
